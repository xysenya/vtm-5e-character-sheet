...
export const FloatingDiceRoller: React.FC<FloatingDiceRollerProps> = ({
  ...
  // Collapsible settings box state: default collapsed on mobile (<768px) to save vertical space
  ...
  return (
    <>
      ...
      {/* Floating Windows (History on the left when open, Roller on the right) */}
      {isOpen && (
        <div
          id="vtm-floating-dice-container"
          className="fixed bottom-16 sm:bottom-20 right-2 sm:right-6 z-50 print:hidden flex flex-col md:flex-row items-end md:items-end gap-2 sm:gap-3 max-w-[calc(100vw-1rem)] sm:max-w-[calc(100vw-2rem)] max-h-[calc(100dvh-4.5rem)] animate-in fade-in slide-in-from-bottom-4 duration-200"
        >
          ...
          {/* Main Roller Column */}
          <div className="w-[295px] xs:w-80 sm:w-96 flex flex-col shrink-0 max-h-[calc(100dvh-4.5rem)]">
            {/* Main Floating Window: "Бросок костей" */}
            <div
              id="vtm-floating-dice-roller"
              className="bg-zinc-950/95 backdrop-blur-md border border-red-900/60 rounded-xl sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden text-zinc-200 max-h-[calc(100dvh-4.5rem)]"
            >
              {/* Header with Title, Mode Switcher, Settings, History Toggle & Close */}
              <div className="bg-gradient-to-r from-red-950/90 via-zinc-900 to-zinc-950 border-b border-red-900/40 px-2.5 sm:px-3.5 py-1.5 sm:py-2.5 flex items-center justify-between gap-1.5 sm:gap-2 shrink-0">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {/* Settings Toggle Button (Automations for Hunger and Willpower) */}
                  <button
                    type="button"
                    id="vtm-toggle-header-settings-btn"
                    ...
                    title={isSettingsOpen ? 'Свернуть панель автоматизации голода и воли' : 'Настроить автоматизацию голода и воли'}
                    aria-label="Настройки автоматизации"
                  >
            ...
            <div className="p-2 sm:p-3.5 space-y-1.5 sm:space-y-3 overflow-y-auto scrollbar-thin flex-1">
              ...
              {/* Settings checkboxes strictly inside the dice roller window (Collapsible) */}
              <div
                id="vtm-roller-settings-bottom-box"
                className="w-full bg-zinc-900/60 border border-zinc-800/90 rounded-lg px-2 sm:px-2.5 py-1 text-zinc-200 transition-all flex flex-col gap-1 shrink-0 mt-1"
              >
                ...
                <button
                  ...
                  onClick={() => setIsSettingsOpen((prev) => !prev)}
                  className="w-full flex items-center justify-between gap-1.5 text-left cursor-pointer group py-0.5 select-none"
                ...
                >
                  <div className="flex items-center gap-1.5 min-w-0">
                    <SlidersHorizontal className="w-3 h-3 text-red-400 shrink-0" />
                    <span className="font-serif text-[10.5px] sm:text-[11.5px] font-semibold text-zinc-300 group-hover:text-white truncate">
                      Автоматизация (голод и воля)
                    </span>
                    <span className="text-[9px] px-1 py-0.1 rounded font-mono bg-zinc-950 border border-zinc-800 text-zinc-400 shrink-0">
                ...
                <AnimatePresence>
                  {isSettingsOpen && (
                    <motion.div
                      ...
                      className="overflow-hidden flex flex-col gap-1.5 pt-1 border-t border-zinc-800/80"
                    >
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        <input
                          ...
                          className="w-3.5 h-3.5 rounded border-red-800 bg-zinc-900 text-red-600 focus:ring-red-500 focus:ring-offset-zinc-950 cursor-pointer shrink-0 accent-red-600"
                        />
                        <span className="font-serif text-[10px] sm:text-[11px] leading-snug text-zinc-300">
                          Авто-увеличение Голода при неудаче Воззвания
                        </span>
                        <Droplet
                          className={`w-3 h-3 shrink-0 ml-auto transition-colors ${
                      ...
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        <input
                          ...
                          className="w-3.5 h-3.5 rounded border-amber-800 bg-zinc-900 text-amber-500 focus:ring-amber-500 focus:ring-offset-zinc-950 cursor-pointer shrink-0 accent-amber-600"
                        />
                        <span className="font-serif text-[10px] sm:text-[11px] leading-snug text-zinc-300">
                          Авто-заполнение воли при перебросе
                        </span>
                        <RotateCcw
                          className={`w-3 h-3 shrink-0 ml-auto transition-colors ${
